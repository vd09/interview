...
vector<edge> primsAlgorithm(vector<list<pair<int,int> > > graph,edge minEdge){
    ...
    while(spanningTree.size() != (n-1) ){
        ...
        visited[minEdge.src] = true;
        visited[minEdge.des] = true;
        ...
        minEdge = Q.top();
        Q.pop();
        while(visited[minEdge.des] && !Q.empty())
        {
            minEdge = Q.top();
            Q.pop();
        }
...
cout <<endl <<"EDEGES ARE:\n";
    for(vector<edge>::iterator x = spanningTree.begin(); x != spanningTree.end(); x++){
